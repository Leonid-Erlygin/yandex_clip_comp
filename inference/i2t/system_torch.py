...
def calculate_loss(loss, device, logits: torch.Tensor) -> Dict[str, torch.Tensor]:
    """Contrastive NCE loss, see https://paperswithcode.com/method/nt-xent for details
    """
    labels = torch.arange(0, logits.shape[0], device=device)
    loss_i2t = loss(logits, labels)
    loss_t2i = loss(logits.T, labels)
...
def calculate_metrics(logits: torch.Tensor) -> Dict[str, torch.Tensor]:
    return {
...
#self.encoders
#self.parameters
def configure_optimizers(hparams, encoders, parameters) -> Tuple[List[Optimizer], List[LRScheduler]]:
    config = hparams.optimization
    param_groups = list()
    used_params = set()
    if 'param_groups' in config:
        for encoder, param_group in config.param_groups.items():
            ...
            for path in param_group.modules:
                module = attrgetter(path)(encoders[encoder])
    ...
    param_groups.append({
        'params': list(set(parameters()) - used_params)
    })
...
def init_pretrain_modules(hparams):
    if 'pretrain' not in hparams:
        return
    ckpt_path = hparams.pretrain.checkpoint_path
