...
class TextModel(nn.Module):
    ...
    def forward(self, text_data) -> torch.Tensor:
        ...
        # ts = self.tokenizer(text_data, padding=True, truncation=True, return_tensors='pt')
        # input_ids = ts['input_ids'].to('cpu')#.to(self.model.device)
        # token_type_ids = ts['token_type_ids'].to('cpu')#.to(self.model.device)
        # attention_mask = ts['attention_mask'].to('cpu')#.to(self.model.device)
